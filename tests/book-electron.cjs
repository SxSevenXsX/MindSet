...
(async()=>{
 ...
 try{
  ...
  for(const mode of ['book','flow','pages']){
   if(mode==='flow')await page.locator('[data-action="toggle-editor-book-view"]').click();
   if(mode==='pages')await page.locator('[data-action="toggle-editor-view"]').click();
  ...
  }checks.push('Repeated Backspace at document start: text, empty paragraph, heading and list in all three views');
  ...
  await page.reload();await page.getByRole('button',{name:'Ouvrir',exact:true}).click();await page.locator('[data-book-info]').waitFor();assert.match(await page.locator('.book-context').first().textContent(),/22 × 14 cm/);assert.ok((await editor.textContent()).includes('Deuxième'));checks.push('Custom landscape paper and note contents survive reopening');
